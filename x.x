...
DESCRIPTION
       ...
       incremented  by  the number of bytes read.  If the file offset is at or past the end of file, no bytes
...
RETURN VALUE
       ...
       is advanced by this number.  It is not an error if this number is smaller than the number of bytes re‐
       quested;  this  may happen for example because fewer bytes are actually available right now (maybe be‐
       cause we were close to end-of-file, or because we are reading from a pipe, or from a terminal), or be‐
       cause read() was interrupted by a signal.  See also NOTES.
...
ERRORS
       ...
       EIO    I/O  error.   This  will  happen for example when the process is in a background process group,
              ...
              reading from a disk or tape.  A further possible cause of EIO on networked filesystems is  when
              an  advisory  lock  had been taken out on the file descriptor and this lock has been lost.  See
...
BUGS
       ...
       atomic  across  threads  (and  processes) are updates of the file offset.  However, before Linux 3.14,
       this was not the case: if two processes that share an open file description (see  open(2))  perform  a
       read()  (or readv(2)) at the same time, then the I/O operations were not atomic with respect to updat‐
       ing the file offset, with the result that the reads in the two processes might  (incorrectly)  overlap
